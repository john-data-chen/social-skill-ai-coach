...
describe("registerSocialSkillsMcp", () => {
  it("registers tools and prompts", () => {
    ...
    const listToolCallback = (mockServer.registerTool as any).mock.calls.find(
      (call: any) => call[0] === "list_social_topics"
    )[2]
    expect(listToolCallback().content[0].type).toBe("text")

    const getKnowledgeCallback = (mockServer.registerTool as any).mock.calls.find(
      (call: any) => call[0] === "get_social_knowledge"
    )[2]
    expect(getKnowledgeCallback({ topics: ["opening"] }).content[0].type).toBe("text")

    const analyzePromptCallback = (mockServer.registerPrompt as any).mock.calls.find(
      (call: any) => call[0] === "analyze_situation"
    )[2]
    expect(analyzePromptCallback({ situation: "test" }).messages[0].role).toBe("user")

    const coachPromptCallback = (mockServer.registerPrompt as any).mock.calls.find(
      (call: any) => call[0] === "coach"
    )[2]
    expect(coachPromptCallback({ situation: "test" }).messages[0].role).toBe("user")

    const roleplayPromptCallback = (mockServer.registerPrompt as any).mock.calls.find(
      (call: any) => call[0] === "roleplay"
    )[2]
    expect(roleplayPromptCallback({ scenario: "test" }).messages[0].role).toBe("user")

    const reflectPromptCallback = (mockServer.registerPrompt as any).mock.calls.find(
      (call: any) => call[0] === "reflect"
    )[2]
